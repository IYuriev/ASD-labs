...
export const drawCircle = (x, y, number) => {
    ctx.beginPath();
...
const directMatrix = (myVariant) => {
    ...
    const k = 1.0 - myVariantArray[2] * 0.01 - myVariantArray[3] * 0.01 - 0.3;
...
export const drawLine = (start, end, drawArrow = false) => {
    const arrowSize = 10;
...
export const drawArc = (start, end, drawArrow = false) => {
    const arrowDistance = 20;
    const bendAngle = Math.PI / 10;
    const arrowSize = 10;
    const midX = (start.x + end.x) / 2;
